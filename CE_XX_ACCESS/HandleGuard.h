...
template <typename T> struct non_zero {
    static bool call(T handle) noexcept {
        return intptr_t(handle) != 0;
    }
};

template <typename T> struct non_negative {
    static bool call(T handle) noexcept {
        return intptr_t(handle) > 0;
    }
};

template <template <typename> typename wrapped_t, typename T> struct with_pseudo_t {
    static bool call(T handle) noexcept {
...
template <template <typename> typename wrapped_t> struct with_pseudo {
    template <typename T> using type = with_pseudo_t<wrapped_t, T>;
};

/// <summary>
/// Strong exception guarantee
/// </summary>
template <typename handle_t, auto close, template <typename> typename is_valid = non_negative> class HandleGuard {
public:
    static constexpr handle_t zero_handle = handle_t(0);

public:
    explicit HandleGuard(handle_t handle = zero_handle) noexcept : _handle(handle) {
    }

    HandleGuard(HandleGuard &&rhs) noexcept : _handle(rhs._handle) {
    ...
    bool valid() const noexcept {
        return is_valid<handle_t>::call(_handle);
    }
    ...
    bool operator==(const HandleGuard &rhs) const noexcept {
